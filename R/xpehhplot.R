xpehhplot <- function(data,plot.pval = TRUE,ylim.scan = 2,pch = 16,cex = 0.5,cex.lab = 1.25,main = NA,cex.main = 1.5,cex.axis=1) {
	if (!(grepl("XPEHH",colnames(data)[3]))) {stop("Unrecognized column name for XPEHH: the data is not a matrix generated by the ies2xpehh() function...")} 
	if (colnames(data)[4] == "-log10(p-value) [bilateral]") {
		yleg = expression("-"*log[10]*"[1"~"-"~"2"~"|"~Phi[scriptstyle(italic(XPEHH))]~"-"~0.5*"|]")
	}
	...
	if (nbr_chrm > 1) {
		...
		plot(pos,data[,3],pch = pch,cex = cex,las = 1,col = col_chr,xaxt = "n",xlab = "Chromosome",ylab = expression(italic(XPEHH)),cex.lab = cex.lab,main = main,cex.main = cex.main,cex.axis=cex.axis)
		...
		par(mar = c(5,5,4,2) + 0.1)
		plot(pos / scale,data[,3],pch = pch,col = col_chr,xlab = paste("Position",unit),ylab = expression(italic(XPEHH)),cex.lab = cex.lab,main = main,cex.main = cex.main,cex.axis=cex.axis)
		abline(h = c(-ylim.scan,ylim.scan),lty = 2)
		if (plot.pval) {
		    if (!is.null(names(dev.list())) && ((names(dev.cur()) == "windows") | (names(dev.cur()) == "X11") | (names(dev.cur()) == "quartz"))) {
   		    		dev.new()
