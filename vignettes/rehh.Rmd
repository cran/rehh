---
title: "Vignette for package *rehh* (version 3.1.x)"
...
```{r minimalcodeexample, results = "hide"}
hh <-                  # data input
  data2haplohh(
...
- `limhomohaplo` sets a minimum number of homozygous chromosomes below which calculation of *EHH* is stopped (or not even started). Its default (and minimum) value is 2. This number should be increased to 4 for small samples of unphased haplotypes in order to limit the influence of a single pair of shared haplotypes (see section \@ref(phasing).)
...
- by default the *EHH* curve is defined by linearly interpolating *EHH* values between consecutive markers, yielding a continuous curve. However in particular for full re-sequencing data, it is more accurate to let this function decrease step-wise at each marker by setting `interpolate`to `FALSE` (although the effect is likely to be minor).
