---
title: "Vignette for package *rehh* (version 3.0)"
...
- support for input files in *variant call format* as well as the output format of the simulation program *ms* [@Hudson2002]. 
...
- additional options to yield virtually identical results with the program *hapbin* (see section \@ref(hapbin)).
...
**Due to changes in the API, although mostly small, this version is not compatible with versions up to and including 2.0.4!**
...
### Input files 
  ...
  - Input files in different formats that originate from a study on the "Creole cattle breed from Guadeloupe" (CGU) [@Gautier2011]. All files contain the same set of phased SNPs of *Bos taurus* chromosome 12 from 140 individuals.
...
The package *rehh* requires as input:
 ...
 - a haplotype data file for each population of interest (see section \@ref(hapfile)).
...
-  a *transposed* format with haplotypes in columns and markers in rows as in the example file `bta12_cgu.thap`. This format is similar to the one produced by the phasing program *SHAPEIT2* [@OConnell2014] (see section \@ref(LoadDataEx2)). This format assumes neither row nor column names and hence no haplotype identifiers can be specified.
 ...
 - *variant call format (vcf)*, comprising both haplotype and marker information. In order to read files in this format, the package *vcfR* has to be installed (see section \@ref(LoadDataEx4)).
 - The output of the simulation program *ms* [@Hudson2002] and its derivatives *msHOT* [@Hellenthal2007],  *ms^2^* [@Ewing2010] and *ms'* [@Kelleher2016]. In order to read these files, the package *gap* has to be installed (see section \@ref(LoadDataEx5)).
...
In order to process *vcf* files, the package [vcfR](https://cran.r-project.org/package=vcfR) has to be installed. 
...
In order to read this format, the package [gap](https://cran.r-project.org/package=gap) has to be installed. 
...
\end{equation*}
Thus $\mathrm{nEHHS}_{s,t}$ is just normalized in order to yield 1 at the focal marker $s$. Note that the normalization factor depends on the frequency of the alleles at the focal marker and consequently is in general not the same for different focal markers.
...
In the case of unphased chromosomes from diploid individuals (see section \@ref(phasing)) *EHHS* can be calculated like *EHH* using equation \@ref(eq:ehh2)  without the restriction to core alleles: 
\begin{equation}
\mathrm{EHHS}_{s,t}=\frac{I_{s,t}}{I_{s,s}}=\frac{n-K_{s,t}}{\frac{1}{2}n}\;.
(\#eq:ehhs2)
\end{equation}
...
### Definition {#ihs}
The abbreviation *iHS* refers to "integrated haplotype homozygosity score". Let *uniHS* represent the un-standardized log-ratio of ancestral *iHH*$_a$ to derived *iHH*$_d$ of a certain focal marker $s$:
$$\mathrm{uniHS}(s)=\ln\left(\frac{\mathrm{iHH}_a(s)}{\mathrm{iHH}_d(s)}\right)$$
...
Candidate regions as obtained by the function `calc_candidate_regions()` can be added to the plot.
...
```{r qqman, fig.align = 'center', fig.width = 7, fig.lp = 'fig:', fig.cap = 'Graphical output of the qqman::manhattan() function', fig.pos = '!h', message = FALSE}
library(qqman)
...
\clearpage
# Furcation patterns and lengths of shared haplotypes
...
```{r newick, fig.align = 'center', fig.width = 6, fig.height = 6, fig.lp = 'fig:', fig.cap = 'Graphical output of the ape::plot.phylo() function', fig.pos = "!h"}
library(ape)
...
- since the original approach of [@Sabeti2002] was not to compare *extended haplotype homozygosity* between *core alleles* of a single SNP, but between multiple *core haplotypes* defined by several neighboring SNPs; such a partition could be created in *rehh* by adding an artificial multi-allelic marker.
...
In the presumably rare cases where phasing is not feasible, *EHH* or *EHHS* can only be meaningfully estimated by reducing the set of evaluated chromosomes to those of homozygous (at the focal marker) individuals and calculating the decay of homozygosity for each individual separately. For unphased haplotypes

- the option `phased` of the functions `calc_ehh()`, `calc_ehhs()` and `scan_hh()` must be set to `FALSE`. If the data is actually phased, however, this entails a substantial loss of information.
- in function `ihh2ihs()`, hence for a within-population scan using *iHH* values, the data should not only be filtered by the frequency of core alleles (parameter `min_maf`) but by the number of evaluated haplotypes, too (parameter `min_nhaplo`).

Although [@Tang2007] reported a moderately high correlation of genome-wide values for phased vs unphased *inES* values for a particular data set, the application of *rehh* to unphased data should be considered still experimental insofar as more formal inquiries have not been undertaken so far.

## Dealing with unpolarized data {#polarizing}
The designation of alleles as 'ancestral' or 'derived' is referred to as *polarization*. Since sequences of ancient genomes are available only for a few species and restricted to a limited time back to the past, the 'ancestral' allele is usually inferred to be the one carried by one or more outgroup species such as chimpanzees or gorillas for humans. However, this presupposes the existence of a reference sequence of suitable 'neighbor' species of sufficient quality as well as reliable genome-wide alignments. Both requirements are not trivial and even if they are fulfilled, any alignment will not cover the whole genome and the covered part will contain mis-specified alleles due to invisible secondary or back-mutations [@Baudry2003], causing spurious signals of selection [@Hernandez2007]. 
...
- since in this case the internal coding of alleles as 0,1,2 etc. conveys no information about ancestral status, it is appropriate to set the parameter `polarized` in function `scan_hh` to `FALSE`: *iHH* values are then computed for the major (most frequent) and minor (second-most frequent) alleles. Although the power to detect selection is not increased by this setting, the results become more reproducible since the reported values do not depend on an arbitrary allele coding. 
...
While [@Voight2006] observed the strong frequency-dependence of expected *iHS* values under neutrality, no equivalent investigations seem to have been undertaken for the cross-population statistics *rsb* and *XP-EHH* and no frequency-dependent standardizations (which would need 2-dimensional bins) were proposed. 

The calculation of *iHS* using unpolarized alleles should be considered still experimental insofar as formal studies on the effectiveness to detect selection are as yet lacking.
...
7. If run in default mode, *hapbin* calculates *EHH* by (notation as in section \@ref(EHH))
\begin{equation*}
